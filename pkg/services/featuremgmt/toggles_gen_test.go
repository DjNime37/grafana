...
func TestFeatureToggleFiles(t *testing.T) {
	...
	t.Run("verify files", func(t *testing.T) {
		...
		t.Run("react openfeature manifest", func(t *testing.T) {
			generateFile(t,
				"openfeature-manifest-react.gen.json",
				generateOpenFeatureManifest(GenerateReact),
			)
		})
	})
}
...
var ignoreFlagNameChecks = []string{
	// new flags using OpenFeature that don't use namespaced names
	"lokiShardSplitting",
...
func verifyFlagName(flag FeatureFlag) error {
	...
	if !isLegacy && !strings.Contains(name, ".") && !slices.Contains(ignoreFlagNameChecks, name) {
...
func generateOpenFeatureManifest(generationTarget FlagGenerationTarget) string {
	type flagDef struct {
